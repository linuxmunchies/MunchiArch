...
# ===================================================================
# Arch Linux Setup Script - Renovated Version
# ===================================================================
...
# NFS Configuration
# TODO: Replace with your NFS server's IP and share paths
readonly NFS_SERVER="YOUR_NFS_SERVER_IP"
readonly NFS_SHARES=(
    "${NFS_SERVER}:/path/to/your/share1|/mnt/local_mount1"
    "${NFS_SERVER}:/path/to/your/share2|/mnt/local_mount2"
)


# Game Drive Configuration
# --- This is set dynamically by the user interface later in the script
GAME_DRIVE_UUID=""
...
optimize_mirrors() {
    ...
    local countries=("United States" "Canada" "Mexico" "United Kingdom" "Germany" "France")
...
setup_timezone() {
    log INFO "Setting timezone to America/Chicago..."

    if ! timedatectl set-timezone America/Chicago; then
...
setup_nfs_mounts() {
    ...
    for share in "${NFS_SHARES[@]}"; do
        IFS='|' read -r server_path local_path <<< "$share"
        setup_single_nfs_mount "$server_path" "$local_path"
    done
}

setup_single_nfs_mount() {
    local server_path="$1"
    local local_path="$2"
    local mount_options="defaults,noatime,soft,intr,rsize=8192,wsize=8192,timeo=14"

    log INFO "Setting up NFS mount: $server_path -> $local_path"
    ...
    if ! showmount -e "$NFS_SERVER" &>/dev/null; then
        log WARN "Cannot connect to NFS server $NFS_SERVER, skipping mount setup"
        return 1
    fi

    # Add to fstab if not already present
    local fstab_entry="$server_path $local_path nfs $mount_options 0 0"

    if ! grep -q "$server_path.*$local_path" /etc/fstab; then
        echo "$fstab_entry" >> /etc/fstab
        log INFO "Added NFS mount to fstab"
    fi

    # Attempt to mount
    if mount "$local_path" &>>"$LOG_FILE"; then
        log INFO "NFS mount successful: $local_path"
        chown "$ACTUAL_USER:$ACTUAL_USER" "$local_path"
    else
        log WARN "Failed to mount $local_path immediately (will try on next boot)"
    fi
}
...
setup_kickstart_nvim() {
    log INFO "Setting up kickstart.nvim..."

    local nvim_config_dir="$ACTUAL_HOME/.config/nvim"

    # Backup existing configuration
    if [[ -d "$nvim_config_dir" ]]; then
        local backup_dir="$nvim_config_dir.backup.$(date +%Y%m%d%H%M%S)"
        sudo -u "$ACTUAL_USER" mv "$nvim_config_dir" "$backup_dir"
        log INFO "Backed up existing nvim config to $backup_dir"
    fi

    # Clean data directories
    sudo -u "$ACTUAL_USER" rm -rf \
        "$ACTUAL_HOME/.local/share/nvim" \
        "$ACTUAL_HOME/.local/state/nvim" \
        "$ACTUAL_HOME/.cache/nvim"

    # Install kickstart.nvim
    sudo -u "$ACTUAL_USER" git clone https://github.com/nvim-lua/kickstart.nvim.git "$nvim_config_dir"

    log INFO "kickstart.nvim installed successfully"
}
...
install_munchiehud_configs() {
    ...
    # --- chown is no longer needed as files are created by the correct user
...
download_zshrc() {
    log INFO "Downloading latest .zshrc configuration..."

    local zshrc_path="$ACTUAL_HOME/.zshrc"

    # Backup existing .zshrc
    if [[ -f "$zshrc_path" ]]; then
        local backup_file="$zshrc_path.backup.$(date +%Y%m%d%H%M%S)"
        sudo -u "$ACTUAL_USER" cp "$zshrc_path" "$backup_file"
        log INFO "Backed up existing .zshrc to $backup_file"
    fi

    # Download new .zshrc
    if sudo -u "$ACTUAL_USER" curl -fsSL "https://raw.githubusercontent.com/linuxmunchies/.zshrc/refs/heads/main/.zshrc" -o "$zshrc_path"; then
        log INFO ".zshrc downloaded and installed successfully"
    else
        log ERROR "Failed to download .zshrc"
        return 1
    fi
}
...
# ===================================================================
# SPECIALIZED SETUPS (CONTINUED)
# ===================================================================

# --- This function creates a very specific directory structure.
# --- It has been commented out to make the script more generic.
# --- Users can uncomment and customize it for their own needs.
...
display_summary() {
   ...
   echo "   • System report: $ACTUAL_HOME/Desktop/system_setup_report.txt"
   echo "   • Backups: $BACKUP_DIR"
   echo ""
   echo "🔄 Post-Setup Actions Required:"
   echo "   • Reboot to activate all changes"
   echo "   • Configure Timeshift snapshots via GUI"
   echo "   • Set up Mullvad VPN credentials"
   echo "   • Configure NFS mounts if needed"
   echo ""
   echo "💡 Tips:"
   echo "   • Check mounted drives: df -h"
   echo "   • Update firmware: fwupdmgr update"
   echo "   • View system info: fastfetch"
   echo ""
...
get_user_choices() {
    ...
    if $use_whiptail; then
        local step_choices
        step_choices=$(whiptail --title "Setup Steps" --checklist "Select steps to run (use space to select):" 20 70 10 \
            "protondrive" "ProtonDrive setup" ON \
        ...
        echo "Select steps to run (comma separated, e.g. essentials,coding,media):"
        echo "Options: protondrive, gamedrive, nfs, essentials, coding, media, gaming, browsers, office, virtualization"
        read -rp "Steps: " steps
        ...
        if [[ ${#drives[@]} -eq 0 ]]; then
            ...
            if $use_whiptail; then
                ...
                select uuid in "${drives[@]}"; do
                    USER_GAMEDRIVE_UUID="$uuid"; break
                done
            fi
        fi
    fi
...
# Call user choices at the start of main
# ===================================================================
# MAIN EXECUTION FLOW
# ===================================================================
...
main() {
   ...
   setup_aur_helper
   setup_flatpak
   # --- Call to create_directory_structure is commented out
   # if [[ " ${USER_STEPS[*]} " =~ protondrive ]]; then
   #     create_directory_structure
   # fi

   # Storage setup phase
   log INFO "=== STORAGE SETUP PHASE ==="
   if [[ " ${USER_STEPS[*]} " =~ nfs ]]; then
       setup_nfs_mounts || log WARN "NFS setup failed - check server connectivity"
   fi
   if [[ " ${USER_STEPS[*]} " =~ gamedrive ]]; then
   ...
   if [[ " ${USER_STEPS[*]} " =~ essentials ]]; then
       show_progress $((++current_step)) $total_steps "Installing essential applications"
       install_essential_apps
   fi
   ...
   if [[ " ${USER_STEPS[*]} " =~ media ]]; then
       show_progress $((++current_step)) $total_steps "Installing multimedia applications"
       install_multimedia_apps
   fi
   if [[ " ${USER_STEPS[*]} " =~ gaming ]]; then
       show_progress $((++current_step)) $total_steps "Installing gaming applications"
       install_gaming_apps
       install_munchiehud_configs
   fi
   if [[ " ${USER_STEPS[*]} " =~ browsers ]]; then
       show_progress $((++current_step)) $total_steps "Installing browsers"
       install_browsers
   fi
   if [[ " ${USER_STEPS[*]} " =~ office ]]; then
       show_progress $((++current_step)) $total_steps "Installing office applications"
       install_office_apps
   fi
   if [[ " ${USER_STEPS[*]} " =~ virtualization ]]; then
       show_progress $((++current_step)) $total_steps "Setting up virtualization"
       setup_virtualization
   fi
   show_progress $((++current_step)) $total_steps "Installing specialized applications"
